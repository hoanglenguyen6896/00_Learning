...
def get_fw_type(ip, name):
    ...
    for line in ssh_stdout.readlines():
        if line == "agv_x2_fw\n":
            ssh.close()
            # print(f"Bot {name}: Old")
            return("Old", 1)
        if line == "agv_x2_executive\n":
            ssh.close()
            # print(f"Bot {name}: New")
            return("New", 1)
    ssh.close()

def print_fw_new(ip, name):
    fw_t, val = get_fw_type(ip, name)
    if fw_t == "New":
        print(name)
...
def power_cycle_with_check(ip, name, decide):
    if decide == "No":
        fw_type, val = get_fw_type(ip, name)
        if fw_type == "New":
...
if __name__ == '__main__':
    # import threading
    # t = threading.Thread(target = power_cycle, args = ("172.16.28.89",))
    # t.start()
    # while t.is_alive() == True:
    #     print(1)
    #     time.sleep(1)
    get_log("172.16.28.140",".","test","sub")
    # get_log("172.16.28.103",".","test","sub")
    pass