...
# Unpause a bot (use post/iherbdriver - cmd can be check from app_v10's config.json)
...
def unpause(bot_name):
    try:
        resp = requests.put(cmd_url["unpause"] + bot_name + cmd_url["unpause_end"], timeout = rq_timeout)
    except requests.exceptions.Timeout:
        return ('Server timeout', 998)
    except:
        return ('Post Error ...', 999)
...
def pause(bot_name):
    try:
        resp = requests.put(cmd_url["pause"] + bot_name + cmd_url["pause_end"], timeout = rq_timeout)
    except requests.exceptions.Timeout:
        return ('Server timeout', 998)
    except:
        return ('Post Error ...', 999)
...
def replan(bot_id):
    cmd = cmd_url["replan_start"] + bot_id + cmd_url["replan_end"]
...
def setidle(pack_id):
    cmd = cmd_url["setidle_start"] + pack_id + cmd_url["setidle_end"]
...
def refreshhmi(pack_id):
    cmd = cmd_url["rfHMI_start"] + pack_id + cmd_url["rfHMI_end"]
    headers = {"accept":"application/json", "Content-Length":"0"}
    try:
        resp = requests.post(cmd, headers=headers, timeout = rq_timeout)
    except requests.exceptions.Timeout:
