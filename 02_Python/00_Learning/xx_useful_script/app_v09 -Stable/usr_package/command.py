...
def global_bot_dir(bot_info, cwd):
    del_dir = cwd + "\\bot_dir"
    os.system(del_dir)
    try:
        os.mkdir(del_dir)
    ...
    for bot, data in bot_info.items():
        if data[info["Status"]] == "Available":
            t = threading.Thread(target = fw.get_bot_dir, args = (data[info["IP"]], str(bot), del_dir))
            t.start()
...
def help_menu():
    ...
    print(f"{'':=<8}{'  U': <10}: {'Quick un-ignore all ignored bots': <52}{'':=<8}")
...
def execute_cmd(bot_info, location, idle_list, pause_list, ignore_list, cwd):
    ...
    while True:
        ...
        elif action.upper() == "GBOTDIR":
            global_bot_dir(bot_info, cwd)
